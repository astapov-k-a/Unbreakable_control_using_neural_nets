function [re lastblank re2]=generate_wave2(min_waves,max_waves,minwlength,maxwlength,interval,minpikes,maxpikes,growth)
%growth - направление роста частоты пиков
if ~exist('growth') growth=1; end
...
m(2,:)=(rand(1,numw+1).*(1:numw+1).*(maxwlength-minwlength+1))+minwlength; %длины волн
m=floor(m);
m(2,numw+1)=0;

ostat=interval-sum(m(2,:));
m(1,:)=rand(1,numw+1);
m(1,:)=m(1,:).*(0:1/(size(m,2)-1):1);
koef=sum(m(1,:))./ostat;
m(1,:)=m(1,:)./koef;
m(1,:)=floor(m(1,:));
m(1,numw+1)=0;
lastblank=interval-sum(sum(m));
m(1,numw+1)=lastblank;
m(1,:)=m(1,(size(m,2):-1:1));
lastblank=m(1,numw+1);
m(1,numw+1)=0;
%m=rand(2,numw); 
